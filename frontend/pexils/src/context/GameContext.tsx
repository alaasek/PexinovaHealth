...
export const GameProvider = ({ children }: { children: ReactNode }) => {
    ...
    const updateMedicineTime = (id: number, time: string) => {
        setMedicines(prev => prev.map(med => {
            if (med.id === id) {
                // Parse new time to minutes for logic
                let minutes = 0;
                try {
                    ...
                    console.warn("Time parsing failed in update", e);
                }
                return { ...med, time, minutes };
            }
            return med;
        }));
    };
    ...
    return (
        <GameContext.Provider value={{ medicines, score, bullets, markMedicineTaken, addMedicine, shootAlien, resetGame, updateMedicineTime }}>
            {children}
