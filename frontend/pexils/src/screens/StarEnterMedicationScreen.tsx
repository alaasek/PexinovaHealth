...
export default function StarEnterMedicationScreen() {
    const navigation = useNavigation<any>();
    const route = useRoute<any>();
    const { email, password, name, dob, disease } = route.params || {};
    ...
    const handleNext = () => {
        // Pass encoded json string if needed, or just the object since React Navigation handles objects well usually. 
        // User code used JSON stringification, let's stick to standard params first.
        navigation.navigate('ConfirmSignup', {
            ...
            dob,
            disease,
            medications: JSON.stringify(meds) // Keeping consistent with user logic 
        });
    };
...
const styles = StyleSheet.create({
    title: { fontSize: 22, fontWeight: '700', color: '#fff', marginBottom: 12, fontFamily: 'Poppins-Bold' },
    input: { width: '100%', backgroundColor: 'rgba(255,255,255,0.06)', padding: 12, borderRadius: 8, color: '#fff', marginBottom: 10, fontFamily: 'Poppins-Regular' },
    ...
    catText: { color: '#fff', fontFamily: 'Poppins-Regular' },
    addButton: { backgroundColor: '#444', padding: 10, borderRadius: 8, alignItems: 'center', marginBottom: 12 },
    addButtonText: { color: '#fff', fontFamily: 'Poppins-Regular' },
    ...
    medName: { color: '#fff', fontWeight: '600', fontFamily: 'Poppins-SemiBold' },
    medMeta: { color: '#ccc', fontSize: 12, fontFamily: 'Poppins-Regular' },
